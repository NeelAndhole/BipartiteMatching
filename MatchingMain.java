  ...
  public static void main(String[] args) {
    ...
    for (int m = 0; m < numOfInstances; m++) {
      ...
      int numOfNodesInB;
      {
        ...
        numOfNodesInA = Integer.parseInt(line[0]);
        numOfNodesInB = Integer.parseInt(line[1]);
        numOfEdges = Integer.parseInt(line[2]);
      }

      Node[] nodes = new Node[numOfNodesInA + numOfNodesInB + 2];
      ...
      for (int i = 0; i < numOfNodesInA; i++) {
        nodes[0].edges.add(new Edge(nodes[0], nodes[i + 1], 1));
      }
      for (int i = 1; i <= numOfNodesInB; i++) {
        nodes[i + numOfNodesInA].edges
            .add(new Edge(nodes[i + numOfNodesInA], nodes[nodes.length - 1], 1));
      }


      for (int i = 0; i < numOfEdges; i++) {
        String[] line = kb.nextLine().split(" ");
        nodes[Integer.parseInt(line[0])].edges.add(new Edge(nodes[Integer.parseInt(line[0])],
            nodes[Integer.parseInt(line[1]) + numOfNodesInA], 1));
      }
      ...
      System.out.println(
          maxFlow + " " + ((maxFlow == numOfNodesInA && maxFlow == numOfNodesInB) ? "Y" : "N"));
